...
static Integer roomNumber;     
	...
	static void mainMenu() {
		...
		//Title label
		getTitle(520,"MAIN ROOM");
		//Button that takes admin to the main menu.
		adminMenuBTN(50);
		getSearchTB(900,50, null);
		getLnameLB(750,55,"Search for an employee");
		mapSearchBTN();
		previousRoomBTN();
		nextRoomBTN();
		assignSeatBTN();
		removeSeatBTN();
		seatNumberLB(100,70,"Please select a seat number: ");
		SQLTableScripts.GetEmailsMapList();
		SQLTableScripts.GetSeatList("Main_Room");
		getThirdLB(750, 300,"Select an Account: ");
		JavaGraphicsAdmin graphics = new JavaGraphicsAdmin();
		graphics.setSize(1300,1000);
		panel.add(graphics);

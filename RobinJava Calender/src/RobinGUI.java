...
public class RobinGUI {
	...
	static JLabel currentDateAndTime = new JLabel();
	static String CurrentDate = null;
	static Integer CurrentTime = null;
	static String TimePmAm = null;
	static JButton FutureRequestBTN = new JButton(); 
	static String Calanderday = null;
	static JButton dateNextBTN = new JButton();
	static JButton datepreviousBTN = new JButton();
	static LocalDate today = null;
	static LocalDate tomorrow = null;
	static JLabel calanderLB = new JLabel();
	static LocalTime todayTime1 = null;
	static LocalTime todayTime = null; 
	static JLabel setTimeLB = new JLabel();
	static Integer timeHours = null;
	static JButton timeNextBTN = new JButton();
	static JButton timePreviousBTN = new JButton();
	static Integer addMinutes = 0;
	static Integer addHours = 1;
	static LocalTime todayTime1B = null;
	static LocalTime todayTimeB = null; 
	static JLabel setTimeLB2 = new JLabel();
	static Integer timeHours2 = null;
	static JButton timeNextBTN2= new JButton();
	static JButton timePreviousBTN2 = new JButton();
	static Integer addMinutes2 = 0;
	static Integer addHours2 = 1;
	static JButton timeSeatAcceptBTN = new JButton();
	static JButton cancelPriortyRequestBTN = new JButton();
	static String calanderDate = null;
	static String timePicked  = null;
	static String Pagetitle = null;
	static LocalTime hour = null;
	static JLabel FutureRequestFeedBackLB = new JLabel();
	static JLabel priortyStatusLB = new JLabel();
	static LocalTime hour3 = null;
	...
	insertBTN.addActionListener(new ActionListener() {
		...
		createBTN.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				panel.removeAll();
				getFeedbackLB(540, 325, "Account details cannot be empty");
				 CreateEditProfile.mainMenu();
				} else {
		...
		searchBTN.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				getFeedbackLB(570, 525, "Please enter email address");
				SwingUtilities.updateComponentTreeUI(panel);
			} else {
		...
		//Assigns employee to seat in AdminMap.
				assignSeatBTN.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if (AdminMap.emailValue == null) {
							getFeedbackLB(380, 630, "Please select a current employee from list before assigning.");
							SwingUtilities.updateComponentTreeUI(panel);	
						} else if (seatNumberLB.getText() == "Please select a seat number: ") {
							getFeedbackLB(500, 630, "Please select a seat.");
							SwingUtilities.updateComponentTreeUI(panel);	
						} else if (!(AdminMap.seatValue == null)) {
						panel.removeAll();
						getFeedbackLB(450, 630, "Please empty seat before assigning.");
						//resets email value
						...
						panel.removeAll();
						AdminMap.getFeedbackLB(500, 630, "Please remove employee from assigned seat before assigning.");
						AdminMap.getRoom();
						SwingUtilities.updateComponentTreeUI(panel);	
					}
					}
				...
				removeSeatBTN.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
					 if (AdminMap.seatNumberLB.getText() == "Please select a seat number: ") {
						...
						panel.removeAll();
						AdminMap.getRoom();
						getFeedbackLB(500, 630, "Seat is already empty.");
						//resets email value
						AdminMap.emailValue = null;
						AdminMap.employeeAssigned = null;
						AdminMap.seatValue = null;
				...
				empRequestBTN.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
					 if (AdminMap.seatNumberLB.getText() == "Please select a seat number: ") {
						 getFeedbackLB(200,600,"Please select a seat before requesting a seat.");
							SwingUtilities.updateComponentTreeUI(panel);	
						} else {
				...
				cancelEmpRequestBTN.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
					 if (EditProfile.changeRequestDetail.equals("2")) {
						 getFeedbackLB(200,600,"Cancel request failed. No request active.");
						 SwingUtilities.updateComponentTreeUI(panel);
						} else {
					 SQLRemoveScripts.removeRequestedSeat(EditProfile.employeeUsername);	
					 panel.removeAll();
					 EmployeeMap.getRoom();
					 getFeedbackLB(225,600,"Request cancelled successfully!");
						}
					}	
			});	
				...
				adminEmpRequestBTN.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) { 
						...
						if (AdminMap.RequestedEmail == null && titlePageLB.getText() == "Current Request Page") {
							getFeedbackLB(580,490,"Please select a user.");
							SwingUtilities.updateComponentTreeUI(panel);
						} else if (AdminMap.RequestedEmail != null && titlePageLB.getText() == "Current Request Page"){	
				...
				adminCancelEmpRequestBTN.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e)  {
						if (AdminMap.RequestedEmail == null && titlePageLB.getText() == "Current Request Page") {
							...
							RequestPage.mainMenu();							
						}	else if (AdminMap.RequestedEmail != null  && titlePageLB.getText() == "Scheduled Request Page") {
							SQLRemoveScripts.deleteFutureRequest2();
						} 
						else if (AdminMap.RequestedEmail == null  && titlePageLB.getText() == "Scheduled Request Page") {
							getFeedbackLB(580,490,"Please select a seat.");
							SwingUtilities.updateComponentTreeUI(panel);
						}
					}	
			});			
	...
	//Button for employee page that changes request view
	FutureRequestBTN.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e)  {
			if (titlePageLB.getText() == "Current Request Page" ) {
				panel.removeAll();
				RequestPage.ScheduledRequestPage();
			} else if (titlePageLB.getText() == "Scheduled Request Page")  {
				panel.removeAll();
				RequestPage.mainMenu();
			}
		}
	...
	dateNextBTN.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e)  {
					
				if (!tomorrow.equals(today.plusDays(14))) {
				EmployeeMap.day  = EmployeeMap.day + 1;
				panel.remove(calanderLB);
				getCalendarLB(400,EmployeeMap.day);
				}
			}
		});	
	...
	datepreviousBTN.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e)  {
	...
	timeNextBTN.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e)  {	
					addMinutes = addMinutes + 30;
					if (hour.toString().equals("23:30")) {
						dateNextBTN.doClick();
					}
					panel.remove(setTimeLB); 
					panel.add(setTimeLB);
					getSetTimeLB();
			}
		});	
	...
	timePreviousBTN.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e)  {
			addMinutes = addMinutes - 30;
			System.out.println(hour);
			if (hour.toString().equals("00:00")) {
				datepreviousBTN.doClick();
			}
			panel.remove(setTimeLB); 
			panel.add(setTimeLB);
			getSetTimeLB();
		...
		timeNextBTN2.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e)  {
					
					if (addMinutes2 < 180) {
						addMinutes2 = addMinutes2 + 30;
						panel.remove(setTimeLB2); 
						panel.add(setTimeLB2);
						getSetTimeLB2();
		}
				}
			});	
		...
		timePreviousBTN2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e)  {
		...
		timeSeatAcceptBTN.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e)  {
				
				if (EmployeeMap.employeeSeatNumber != null && tomorrow.getDayOfWeek().equals(today.getDayOfWeek()) && (LocalTime.now().isBefore(hour) || LocalTime.now().equals(hour))) {
					SQLSelectScripts.CheckFutureRequestList();
					 SwingUtilities.updateComponentTreeUI(panel);	
				} else if (EmployeeMap.employeeSeatNumber != null && !tomorrow.getDayOfWeek().equals(today.getDayOfWeek())) {
					SQLSelectScripts.CheckFutureRequestList();
					 SwingUtilities.updateComponentTreeUI(panel);	
				} else if (EmployeeMap.employeeSeatNumber == null){
					FutureRequestFeedBackLB(215, 670, "Please make sure you selected a seat.");
   					SwingUtilities.updateComponentTreeUI(panel);
				}
   					else  {
   						FutureRequestFeedBackLB(215, 670, "Please set a time after currentTime ");
   					 SwingUtilities.updateComponentTreeUI(panel);	
   					}
			}
		});	
		...
		cancelPriortyRequestBTN.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e)  {
				SQLSelectScripts.CheckFutureRequestList3();
				 SwingUtilities.updateComponentTreeUI(panel);	
	...
	static void getWelcomeLB()  {
		...
		welcomeLB.setBounds(550,555,250,30);
		panel.add(welcomeLB);
	...
	//Method that displays the welcome label
		static void getCurrentDateTime(int YValue)  {
			//Forrmat to display the date and time.
			DateTimeFormatter dateFormat = DateTimeFormatter.ofPattern("MM/dd");
			DateTimeFormatter timeFormat = DateTimeFormatter.ofPattern("hh:mm");
			//Variable that get the current date or time and displays it in the format you created.
			LocalDateTime date = LocalDateTime.now();
			LocalDateTime time = LocalDateTime.now();
			
			currentDateAndTime.setText("Current date: " + dateFormat.format((date)) + " time: " + timeFormat.format(time));
			currentDateAndTime.setFont(new Font ("Serif", Font.PLAIN, 16));
			currentDateAndTime.setBounds(50,YValue,450,50);
			panel.add(currentDateAndTime);
			
			
		
		}
		
		static void getCalendarLB(int YValue, int a) {
			today =  LocalDate.now(); 
			tomorrow = today.plusDays(a); 	
			calanderDate = tomorrow + ": " + tomorrow.getDayOfWeek();
			calanderLB.setText(calanderDate);
			calanderLB .setFont(new Font ("Serif", Font.PLAIN, 26));
			calanderLB .setBounds(50,YValue,450,50);
			panel.add(calanderLB);
			SwingUtilities.updateComponentTreeUI(panel);	
		}
		
		static void getSetTimeLB()  {
			timeHours = LocalTime.now().getHour();
			
			
			
			hour = LocalTime.MIDNIGHT.plusHours(timeHours + addHours).plusMinutes(addMinutes);	
			int hour2 = LocalTime.now().getMinute();
			System.out.println(hour2);
			if (hour2 >= 30) {
				hour3 = LocalTime.MIDNIGHT.plusHours(timeHours).plusMinutes(30);	
				System.out.println(hour3);
			} else {
				hour3 = LocalTime.MIDNIGHT.plusHours(timeHours);
				System.out.println(hour3);
			}
			
			timePicked = "Choose time: " + hour;
			setTimeLB.setText(timePicked);
			setTimeLB.setFont(new Font ("Serif", Font.PLAIN, 26));
			setTimeLB.setBounds(350,400,450,50);
			panel.add(setTimeLB);
			SwingUtilities.updateComponentTreeUI(panel);	
		}
		
		static void getSetTimeLB2()  {
			timeHours2 = LocalTime.now().getHour();
			LocalTime hour2 = LocalTime.MIDNIGHT.plusMinutes(addMinutes2);	
			String timePicked2 = "Reserve seat for: " + hour2;
			setTimeLB2.setText(timePicked2);
			setTimeLB2.setFont(new Font ("Serif", Font.PLAIN, 26));
			setTimeLB2.setBounds(200,500,450,50);
			panel.add(setTimeLB2);
			SwingUtilities.updateComponentTreeUI(panel);	
		}
		
		static void deleteYesterdayRequest() {
	...
	//Method that displays the welcome label
		static void getCurrentSeatLB()  {
			...
			currentSeatLB.setBounds(530,305,300,30);
			} else {
				currentSeatLB.setText("Current seat: Seat " + employeePage.empSeatid);
				currentSeatLB.setBounds(550,305,300,30);
			}
	...
	//JLabel for most feedback when a button is pressed.
		static void FutureRequestFeedBackLB(int lbX, int lbY, String Label)  {
			FutureRequestFeedBackLB.setBounds(lbX,lbY,400,30);
			FutureRequestFeedBackLB.setText(Label);
				panel.add(FutureRequestFeedBackLB);
					}
		...
		public static void requestLBMethod(int i) {
			...
			requestLB.setText("Number of requests: " + AdminMap.numberOfRequests);
				...
				if (EditProfile.changeRequestDetail.equals("1")) {
					requestLB.setBounds(550, 100, 200, 50);
				requestLB.setText("Approval status: Pending");
				} else {
					requestLB.setBounds(570, 100, 200, 50);
	...
	public static void homebackBTN() {
		homeBTN.setBounds(50, 700, 100, 40);
	...
	public static void loginBTN() {
		loginBTN.setBounds(580, 200, 100, 40);
	...
	public static void adminMenuBTN(int x) {
		adminMenuBTN.setBounds(x, 670, 100, 40);
	...
	public static void employeeMenuBTN() {
		employeeMenuBTN.setBounds(50, 700, 100, 40);
	...
	public static void profileBTN() {
		profileBTN.setBounds(250, 200, 250, 50);
	...
	public static void changeSeatBTN() {
		changeSeatBTN.setBounds(500, 200, 250, 50);
	...
	public static void empLeaveSeatBTN() {
		empLeaveSeatBTN.setBounds(750, 200, 250, 50);
	...
	public static void adminprofileBTN() {
		adminprofileBTN.setBounds(525, 200, 175, 40);
	...
	public static void adminchangeSeatBTN() {
		adminchangeSeatBTN.setBounds(725, 200, 200, 40);
	...
	public static void adminrequestBTN() {
		adminrequestBTN.setBounds(325, 200, 175, 40);
	...
	public static void editBTNMethod() {
		insertBTN.setBounds(555, 275, 200, 40);
	...
	public static void backProfileBTN () {
		backProfileBTN.setBounds(100, 670, 100, 40);
	...
	//Button that takes you back to the Admin profile list page
		public static void adminProfileListBTN () {
			adminProfileListBTN.setBounds(100, 670, 100, 40);
	...
	//Button that takes you to the create profile page
	 public static void createMenuBTN() {
	 	createMenuBTN.setBounds(850, 50, 200, 40);
	 ...
	 public static void createBTN() {
			createBTN.setBounds(540, 275, 200, 40);
	 ...
	 public static void searchBTN() {
			searchBTN.setBounds(660, 550, 200, 40);
	...
	//Buton that insert a new account to the database
		 public static void mapSearchBTN() {
				mapSearchBTN.setBounds(1050, 50, 60, 40);
	 ...
	 public static void nextRoomBTN() {
			nextRoomBTN.setBounds(950, 670, 125, 40);
	 ...
	 public static void previousRoomBTN() {
		 previousRoomBTN.setBounds(820, 670, 125, 40);
	 ...
	 public static void empNextRoomBTN() {
			empNextRoomBTN.setBounds(950, 670, 125, 40);
			empNextRoomBTN.setText("Next Room");
			panel.add(empNextRoomBTN);
			SwingUtilities.updateComponentTreeUI(panel);	
		}
	 
	//Button that moves to the next date in employee Map
		 public static void getDateNextButton() {
			 dateNextBTN.setBounds(190, 450, 65, 40);
			 dateNextBTN.setText("Forward");
				panel.add(dateNextBTN);
				SwingUtilities.updateComponentTreeUI(panel);	
			}
		 
		//Button that moves to the next page in employee Map
		 public static void getDatePreviousButton() {
			 datepreviousBTN.setBounds(125, 450, 65, 40);
			 datepreviousBTN.setText("Previous");
				panel.add(datepreviousBTN);
				SwingUtilities.updateComponentTreeUI(panel);	
			}
	 
		//Button that moves to the next date in employee Map
		 public static void getTimeNextButton() {
			 timeNextBTN.setBounds(450, 450, 65, 40);
			 timeNextBTN.setText("Forward");
				panel.add(timeNextBTN);
				SwingUtilities.updateComponentTreeUI(panel);	
			}
		 
		//Button that moves to the next page in employee Map
		 public static void getTimePreviousButton() {
			 timePreviousBTN.setBounds(385, 450, 65, 40);
			 timePreviousBTN.setText("Previous");
				panel.add(timePreviousBTN);
				SwingUtilities.updateComponentTreeUI(panel);
		 }	
				
				//Button that moves to the next date in employee Map
				 public static void getTimeNextButton2() {
					 timeNextBTN2.setBounds(330, 550, 65, 40);
					 timeNextBTN2.setText("Forward");
						panel.add(timeNextBTN2);
						SwingUtilities.updateComponentTreeUI(panel);	
					}
				 
				//Button that moves to the next page in employee Map
				 public static void getTimePreviousButton2() {
					 timePreviousBTN2.setBounds(265, 550, 65, 40);
					 timePreviousBTN2.setText("Previous");
						panel.add(timePreviousBTN2);
						SwingUtilities.updateComponentTreeUI(panel);	
			}
	 
		
		 public static void getTimeSeatAcceptBTN() {
			 timeSeatAcceptBTN.setBounds(350, 630, 150, 40);
			 timeSeatAcceptBTN.setText("Schedule request");
				panel.add(timeSeatAcceptBTN);
				SwingUtilities.updateComponentTreeUI(panel);	
		 }
		 
		 public static void getPriortyStatusLB(int XValue, int YValue, String status) {
			 priortyStatusLB.setBounds(XValue, YValue, 600, 40);
			 priortyStatusLB.setText(status);
				panel.add(priortyStatusLB);
				SwingUtilities.updateComponentTreeUI(panel);	
		 }
		 
		
		 
		 public static void getCancelPriortyRequestBTN() {
			 cancelPriortyRequestBTN.setBounds(150, 630, 200, 40);
			 cancelPriortyRequestBTN.setText("Cancel Schedule request");
				panel.add(cancelPriortyRequestBTN);
				SwingUtilities.updateComponentTreeUI(panel);	
			}
	 ...
	 public static void empPreviousRoomBTN() {
		 empPreviousRoomBTN.setBounds(820, 670, 125, 40);
	 ...
	 public static void assignSeatBTN() {
		 assignSeatBTN.setBounds(570, 670, 125, 40);
	 ...
	 public static void removeSeatBTN() {
		 removeSeatBTN.setBounds(440, 670, 125, 40);
		 removeSeatBTN.setText("Remove Seat");
		 panel.add(removeSeatBTN);
		 SwingUtilities.updateComponentTreeUI(panel);	
		}
	 
	 public static void getFutureRequestBTN(Integer XValue) {
		 FutureRequestBTN.setBounds(XValue, 450, 160, 40);
		 FutureRequestBTN.setText("Change type of Request");
		 panel.add( FutureRequestBTN);
		 SwingUtilities.updateComponentTreeUI(panel);	
		}
	 
	
	 public static void empRequestBTN() {
		 empRequestBTN.setBounds(270, 300, 125, 40);
	 ...
	 public static void cancelEmpRequestBTN() {
		 cancelEmpRequestBTN.setBounds(140, 300, 125, 40);
	 ...
	 public static void adminCancelEmpRequestBTN(Integer XValue) {
		 adminCancelEmpRequestBTN.setBounds(XValue, 450, 170, 40);
